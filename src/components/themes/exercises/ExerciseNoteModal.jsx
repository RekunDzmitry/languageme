...
export default function ExerciseNoteModal({
  exerciseKey,
  themeId,
  exerciseNote,
  exercisePrompt,
  ...
  const { t } = useT()
  const [value, setValue] = useState(exerciseNote?.content || '')
  ...
  const scheduleAutoSave = useCallback((text) => {
    ...
    debounceTimer.current = setTimeout(() => {
      ...
      if (trimmed && trimmed !== (exerciseNote?.content || '').trim()) {
        handleSave(trimmed)
      }
    }, SAVE_DEBOUNCE_MS)
  }, [exerciseKey, themeId, onSave, exerciseNote?.content])
      ...
      <div className="bg-surface border border-border rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-fade-in">
        ...
        <div className="flex items-center justify-between px-5 py-3.5 bg-white/5 border-b border-white/[0.06]">
          <div className="flex flex-col min-w-0">
            ...
            {exercisePrompt && (
              <span className="text-xs text-text-muted mt-0.5 truncate" title={exercisePrompt}>
                {exercisePrompt}
              </span>
            )}
          </div>
        ...
        <div className="px-5 py-4 space-y-4">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {exerciseNote && !showDeleteConfirm && (
              <button
